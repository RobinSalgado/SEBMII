...
typedef struct
{
	...
	SPI_Type* SPI_Channel;
	SPI_ChannelType SCGC_SPI_channel;
...
static void SPI_enable(SPI_Type* SPI_channel);

/*It activate the clock gating*/
static void SPI_clk(SPI_ChannelType SPI_channel);

/*It configure the SPI as a master or slave depending on the value of masterOrslave*/
static void SPI_setMaster(SPI_Type* SPI_channel, SPI_MasterType masterOrSlave);

/*It activate the TX and RX FIFOs of the SPI depending on the value of enableOrdisable*/
static void SPI_fIFO(SPI_Type* SPI_channel, SPI_EnableFIFOType enableOrDisable);

/*It selects the clock polarity depending on the value of cpol*/
static void SPI_clockPolarity(SPI_Type* SPI_channel, SPI_PolarityType cpol);

/*It selects the frame size depending on the value of frameSize and the macros that are defined above*/
static void SPI_frameSize(SPI_Type* SPI_channel, uint32 frameSize);

/*It selects the clock phase depending on the value of cpha*/
static void SPI_clockPhase(SPI_Type* SPI_channel, SPI_PhaseType cpha);

/*It selects the baud rate depending on the value of baudRate and the macros that are defined above*/
static void SPI_baudRate(SPI_Type* SPI_channel, uint32 baudRate);

/*It selects if MSB or LSM bits is first transmitted*/
static void SPI_mSBFirst(SPI_Type* SPI_channel, SPI_LSMorMSBType msb);

/*It stars the SPI transmission by modifying the value of HALT bit*/
void SPI_startTranference(SPI_Type* SPI_channel);

/*It stops the SPI transmission by modifying the value of HALT bit*/
void SPI_stopTranference(SPI_Type* SPI_channel);

/*It transmits the information contained in data*/
/****ADDED SPI TYPE TO SPECIFY SPI CHANNEL****/
void SPI_sendOneByte(SPI_Type* SPI_channel, uint8 Data);
