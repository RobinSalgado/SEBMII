...
void RTC_init() {
	time.yearAndDate.yearAndDate.days_units = 9;
	time.monthsAndWeekday.monthsAndWeekday.months_tens = 1;
	time.monthsAndWeekday.monthsAndWeekday.months_units = 1;
	time.monthsAndWeekday.monthsAndWeekday.weekday = THURSDAY;
	time.hours.data = 0xD1;
	time.minutes.data =	0x59;
	time.seconds.data = 0x50;
	RTC_setTime(I2CSLAVE_RTC, &time);
...
BCD_struct yearToBCD(RTC_timeStruct_t * time) {
	BCD_struct yearBCD;
	uint16 auxLeapYear = getLeapYear();
	uint16 auxYear = (auxLeapYear * 4) + time->yearAndDate.yearAndDate.year;
...
uint8 setYear(uint16 year, RTC_timeStruct_t * time) {
	...
	if (year % 4 == 0) {
		...
		auxData = RTC_singleRead(I2CSLAVE_RTC, YEAR_DATE);
		auxData &= ~(auxMask);
		RTC_singleWrite(I2CSLAVE_RTC, YEAR_DATE, (auxData | 0));
	...
	if (year % 4 == 0) {
		...
		RTC_singleWrite(I2CSLAVE_RTC, YEAR_DATE, (auxData | 0x40));
		return 1;
	}
	...
	if (year % 4 == 0) {
		...
		RTC_singleWrite(I2CSLAVE_RTC, YEAR_DATE, (auxData | 0x80));
		return 2;
	}
	...
	if (year % 4 == 0) {
		...
		auxData = RTC_singleRead(I2CSLAVE_RTC, YEAR_DATE);
		auxData &= ~(auxMask);
		RTC_singleWrite(I2CSLAVE_RTC, YEAR_DATE, (auxData | 0xC0));
