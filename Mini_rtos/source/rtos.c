...
/*PREGUNTAS  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
 *  (X)EL rtos_task_handle_t QUE TIPO DE DATO ES YA QUE SI ABRO LA DECLARACIÓN NO ME APARECE UNA ESTRUCTURA COMO TAL
 *  llamar rtos_create_task(idle_task,0,auto_start) , NO SE QUE PONER EN EL ARGUMENTO DE AUTO_START*/
struct
{
...
void rtos_start_scheduler(void)
{
	...
	rtos_create_task(idle_task, 0,kAutoStart);// Qúe significa el auto-start
...
if(RTOS_MAX_NUMBER_OF_TASKS >task_list.nTasks) // para que no se creen má tareas de las que se puede
	{
		...
		task_list.tasks[task_list.nTasks].state=
				kStartSuspended == kAutoStart ? S_SUSPENDED: S_READY;
...
{
task_list.tasks[task_list.nTasks].state = S_WAITING; // Pone la tarea que llamó esta función en estado de espera
task_list.tasks[task_list.nTasks].local_tick = ticks;//Asigna ticks a el reloj local de la tarea
...
static void dispatcher(task_switch_type_e type)
{
	...
	int8_t highest = -1;//para que cualquier tarea pueda sustituir a la tarea inválida
	for(index = 0 ; index < task_list.nTasks ; index++) // checamos cual es la tarea de mayor prioridad
	{
